...
#ifndef _SBETOOL_OPTIONALDECIMALENCODING_H_
#define _SBETOOL_OPTIONALDECIMALENCODING_H_
...
public:
    ...
    OptionalDecimalEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        OptionalDecimalEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    OptionalDecimalEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        OptionalDecimalEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    OptionalDecimalEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = OptionalDecimalEncoding(buffer, offset, bufferLength, actingVersion);
    }
...
#pragma clang diagnostic pop
    ...
    OptionalDecimalEncoding &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, OptionalDecimalEncoding writer)
{
    builder << '{';
    builder << R"("mantissa": )";
