...
class Parser:
	...
	def update_enum_attrib(self, type, user_defined_types):
		items = type.items()
		enum_attrib = {}
	...
	def parse_all_enums(self, types, user_defined_types):
		...
		for type in types.iter('enum'):
			...
			enum_attrib = self.update_enum_attrib(type = type, user_defined_types = user_defined_types)
			primitive_encoding_type = self.get_primitive_encoding_type(enum_attrib)
			enum_file = EnumClassFileGen("sbe::test", enum_name, Metadata.c_field_types[primitive_encoding_type]\
				, enum_values, enum_include_list)
	...
	def generate_composite_type(self, msg_gen, composite_name, type, prvious_type_name, user_defined_types\
		, user_defined_enums):
		type_name = type.attrib['name']
		...
		field_attrib =  self.update_type_attrib(field = type, user_defined_types = user_defined_types)
		...
		if(self.is_string_field(primitive_type, field_attrib)):
			if(self.is_const_type(type.attrib)):
				...
				msg_gen.gen_composite_const_string_field_def(message_name = composite_name\
				...
				print ('const string field:', type_name)
				msg_gen.gen_composite_string_field_def(message_name = composite_name\
					, field_type = Metadata.c_field_types[primitive_type]\
					, field_name = type_name, prvious_field_name = prvious_type_name, field_size = type.attrib['length'])
				return type_name
		...
		if(self.is_numeric(primitive_type, type.attrib) == True):
			if(('presence' in type.attrib) and (type.attrib['presence'] == 'constant')):
				msg_gen.gen_composite_const_numeric_field_def(message_name = composite_name\
					, field_type = Metadata.c_field_types[primitive_type]\
					, field_name = type_name, prvious_field_name = prvious_type_name\
					, value = type.text)
				return type_name
				...
				msg_gen.gen_composite_numeric_field_def(message_name = composite_name\
					, field_type = Metadata.c_field_types[primitive_type]\
					, field_name = type_name, prvious_field_name = prvious_type_name\
	...
	def generate_composite(self, composite, user_defined_types, user_defined_enums):
		...
		for type in composite.iter('type'):
			prvious_type_name = self.generate_composite_type(msg_gen, composite_name, type\
				, prvious_type_name, user_defined_types, user_defined_enums)

		msg_gen.gen_end_of_fields()


	def parse_all_composites(self, types, user_defined_types, user_defined_enums):
		user_defined_composites = []
		for composite in types.iter('composite'):
			composite_name = composite.attrib['name']
			user_defined_composites.append(composite_name)

		self.generate_composite(composite, user_defined_types, user_defined_enums)
		return user_defined_composites


	def parse_message_field(self, msg_gen, message_name, field, prvious_field_name, user_defined_types\
	...
	def parse_message(self, message, user_defined_types, user_defined_enums, user_defined_composites):
		...
		for field in message.iterfind('field', namespaces):
			prvious_field_name = self.parse_message_field(msg_gen, message_name, field, prvious_field_name, user_defined_types\
				, user_defined_enums, user_defined_composites)

		msg_gen.gen_end_of_fields()


	def parse_all_messages(self, root, user_defined_types, user_defined_enums, user_defined_composites):
		namespaces = {'sbe': 'http://fixprotocol.io/2016/sbe'}
		for message in root.iterfind('sbe:message', namespaces):
			self.parse_message(message, user_defined_types, user_defined_enums, user_defined_composites)
	...
	def my_main(self):		
		...
		user_defined_types = self.parse_all_types(types = types)
		user_defined_enums = self.parse_all_enums(types = types, user_defined_types = user_defined_types)
		user_defined_composites = self.parse_all_composites(types = types, user_defined_types = user_defined_types\
			, user_defined_enums = user_defined_enums) 
		self.parse_all_messages(root = root, user_defined_types = user_defined_types\
			, user_defined_enums = user_defined_enums, user_defined_composites = user_defined_composites)
